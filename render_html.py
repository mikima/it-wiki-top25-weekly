...
def write_week_html(docs_dir: Path, previous_years: int, json_url_base: str) -> None:
...
<head>
  ...
  <style>
    ...
    .trend-cell svg {{ display: block; width: 120px; height: 28px; }}
    ...
    @media (max-width: 900px) {{
      ...
      .trend-cell svg {{ width: 100px; height: 24px; }}
    }}
  </style>
</head>
<body>
  ...
  <script>
    ...
    function buildBarChart(dailyViews) {{
      ...
      const bars = dailyViews.map((item, index) => {{
        ...
        const barHeight = Math.max(Math.floor(((value - min) / denom) * innerHeight), 1);
    ...
    function renderNav(weekId) {{
      ...
      const html = `
        ...
        <span class="current">${{weekId}}</span>
      ...
      document.title = `Top pagine it.wiki - ${{weekId}}`;
    }}
    ...
    function renderRows(data) {{
      ...
      const html = rows.map((item) => {{
        ...
        const titleCell = item.article_url
          ...
          : titleEsc;
        const trendSvg = buildBarChart(item.daily_views || []);
...
def main() -> int:
    ...
    write_week_html(docs_dir, args.previous_years, args.json_url_base)
    return 0
